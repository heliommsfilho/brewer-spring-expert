...
public class FotoStorageS3 implements FotoStorage {
	...
	private static final String BUCKET = "heliommsfilho-brewer";
	...
	public String salvar(MultipartFile[] files) {
		...
		if (files != null && files.length > 0) {
			...
			try {
				...
				throw new RuntimeException("Erro ao ssalvar arquivo no S3", e);
			}
		}
		return novoNome;
	}
	...
	public byte[] recuperar(String foto) {
		...
		try {
			...
			logger.error("Não foi possível recuperar foto do S3");
		}
	...
	public byte[] recuperarThumbnail(String foto) {
		return recuperar(THUMBNAIL_PERFIX + foto);
	}
	...
	public String getUrl(String foto) {
		...
		if (!StringUtils.isEmpty(foto)) {
			return "https://s3.us-east-2.amazonaws.com/heliommsfilho-brewer/" + foto;
		}
	...
	private void enviarThumbnail(String novoNome, MultipartFile arquivo, AccessControlList acl) throws IOException {
		...
		amazonS3.putObject(new PutObjectRequest(BUCKET, THUMBNAIL_PERFIX + novoNome, is, thumbMetadata).withAccessControlList(acl));
	}
}
